...
def buy_checkout(user_id, book_id):
    ...
    # Get user addresses
    addresses = Address.get_user_address(user_id)
    ...
    print(matching_book)
    return render_template('buycheckout.html', book_detail=book_detail, books=books, matching_book=matching_book, user_id=user_id, addresses=addresses, get_user_address=Address.get_user_address)


@home.route('/<int:user_id>/buy/checkout', methods=['GET'])
def get_user_address(user_id):
    addresses = Address.get_user_address(user_id)

    if addresses:
        return render_template('buycheckout.html', addresses=addresses)
    else:
        return render_template('buycheckout.html', message='User profile not found'), 404
